...
partial class ILImporter
{
    ...
    void CheckIsAssignable(Type src, Type dst)
    {
        if (!IsAssignable(src, dst))
        {
    ...
    void CheckIsArrayElementCompatibleWith(Type src, Type dst)
    {
    ...
    void CheckIsPointerElementCompatibleWith(Type src, Type dst)
    {
    ...
    void CheckIsObjRef(Type type)
    {
        if (!IsAssignable(type, typeof(object), false))
        {
            // TODO: Better error message
    ...
    private void CheckIsNotPointer(Type type)
    {
        if (type.IsPointer)
    ...
    Type GetVarType(int index, bool argument)
    {
        if (argument)
        {
            if (_thisType != null)
    ...
    void ImportReturn()
    {
        ...
        if (_trackObjCtorState)
            Check(_isThisInitialized || !_thisType.IsValueType, VerifierError.ThisUninitReturn);
    ...
    void ImportConvert(Type wellKnownType, bool checkOverflow, bool unsigned)
    {
        var value = Pop();

        CheckIsNumeric(value);

        Push(StackValue.CreateFromType(wellKnownType));
    }
    ...
    void ImportStoreIndirect(Type type)
    {
        ClearPendingPrefix(Prefix.Unaligned);
        ClearPendingPrefix(Prefix.Volatile);
    ...
    void ImportNewArray(int token)
    {
        ...
        Push(StackValue.CreateObjRef(elementType.MakeArrayType()));
    }
    ...
    void ImportLoadElement(Type elementType)
    {
        var index = Pop();
        var array = Pop();
        ...
        if (array.Type != null)
        {
            var actualElementType = array.Type.GetElementType();
    ...
    void ImportStoreElement(Type elementType)
    {
        var value = Pop();
        var index = Pop();
        var array = Pop();
